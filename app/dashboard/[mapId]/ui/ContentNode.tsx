...
import type {
  ...
  BaseOutputNodeData,
  ChatMessage,
  ContentNodeFeatureConfig,
  ContentVersion,
  MessagingLengthOption,
  SentenceChangeKind,
} from './contentNodeTypes';
...
export default function ContentNode({
  ...
  enableMessagingLength = false,
}: NodeProps<BaseOutputNode> & ContentNodeFeatureConfig) {
  ...
  const [platform, setPlatform] = useState<Platform>(lockPlatform ?? data?.platform ?? 'LINKEDIN');
  ...
  const titleLabel = nodeLabel;
  const contextLabel = getContextLabel?.(platform) ?? getPlatformLabel(platform);
  const generateActionLabel = getGenerateActionLabel?.(platform) ?? `Generate ${getPlatformLabel(platform)}`;
  ...
  const headerTagClass = accent === 'indigo' ? 'text-indigo-700' : 'text-primary';
  const activeTabClass = accent === 'indigo'
  ...
  const generateButtonClass = accent === 'indigo'
  ...
  useEffect(() => {
    if (lockPlatform) {
      if (platform !== lockPlatform) {
        setPlatform(lockPlatform);
      }
      if (data?.platform !== lockPlatform) {
        mindmap.updateNodeData(id, { platform: lockPlatform });
      }
      return;
    }
  ...
  }, [data?.platform, id, lockPlatform, mindmap, platform]);

  useEffect(() => {
    if (!isSettingsMenuOpen) return;
      const handleClickOutside = (event: MouseEvent) => {
      const target = event.target as HTMLElement | null;
      if (target?.closest?.(`[data-output-settings="${settingsScope}-${id}"]`)) return;
      setIsSettingsMenuOpen(false);
    };

    document.addEventListener('mousedown', handleClickOutside);
    return () => document.removeEventListener('mousedown', handleClickOutside);
  }, [id, isSettingsMenuOpen, settingsScope]);

  useEffect(() => {
    if (!enableChat && isChatMode) {
      setIsChatMode(false);
      setChatMessages([]);
      setChatInput('');
    }
  }, [enableChat, isChatMode]);

  useEffect(() => {
    if (!enableRefine && isRefineMode) {
      exitRefineModeWithoutSaving();
    }
  }, [enableRefine, isRefineMode]);
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      if (generatedContent) {
        ...
        const versionHistoryAfter = ((nodeAfterGeneration?.data as BaseOutputNodeData)?.versionHistory ?? []) as ContentVersion[];
        const currentVersionAfter = ((nodeAfterGeneration?.data as BaseOutputNodeData)?.currentVersion ?? 1) as number;
            ...
            for (const line of lines) {
              try {
                ...
                if (event.type === 'delta' && event.delta) {
                  ...
                  const versionHistoryAfter = ((nodeAfterChat?.data as BaseOutputNodeData)?.versionHistory ?? []) as ContentVersion[];
                  ...
                  if (versionHistoryAfter.length === 0) {
                    ...
                    // Create new version
                    const currentVer = ((nodeAfterChat?.data as BaseOutputNodeData)?.currentVersion ?? 1) as number;
  ...
  const handleRefineDone = () => {
    ...
    if (trimmedDraft !== trimmedContent) {
      ...
      const versionHistoryAfter = ((nodeAfterRefine?.data as BaseOutputNodeData)?.versionHistory ?? []) as ContentVersion[];
      ...
      if (versionHistoryAfter.length === 0) {
        ...
        // Create new version
        const currentVer = ((nodeAfterRefine?.data as BaseOutputNodeData)?.currentVersion ?? 1) as number;
        const newVersion: ContentVersion = {
          version: currentVer + 1,
          content: persistedContent,
          contentByPlatform: { ...contentByPlatform, [platform]: persistedContent },
  ...
  return (
    ...
    >
      ...
      <Tooltip
        id={`copy-tooltip-${settingsScope}-${id}`}
        place="bottom"
        className={tooltipClassName}
        opacity={1}
        delayShow={80}
      />
      <Handle type="target" position={Position.Left} className={`!h-2.5 !w-2.5 ${accent === 'indigo' ? '!bg-indigo-500' : '!bg-primary'}`} />
      ...
      <div className={`mb-3 flex items-start justify-between gap-2 rounded-2xl border border-slate-200/80 bg-white/85 px-3 py-2 shadow-sm ${isGenerating || isChatLoading ? 'blur-sm opacity-60' : ''}`}>
        <div>
          ...
          <div className="mt-0.5 text-sm font-semibold text-dark">{contextLabel}</div>
        </div>
        <div className="flex items-center gap-2">
          {hasGeneratedContent ? (
            <div className="relative" data-output-settings={`${settingsScope}-${id}`}>
              <button
                type="button"
      ...
      {enableMessagingLength ? (
        <div className={`mb-3 rounded-2xl border border-slate-200/80 bg-white/90 p-3 shadow-sm ${isGenerating || isChatLoading ? 'blur-sm opacity-60' : ''}`}>
          ...
          <div className="relative mb-2">
            ...
            className={`nodrag h-2 w-full appearance-none rounded-lg bg-slate-200 outline-none focus:outline-none disabled:opacity-50 [&::-webkit-slider-thumb]:appearance-none [&::-webkit-slider-thumb]:h-5 [&::-webkit-slider-thumb]:w-5 [&::-webkit-slider-thumb]:rounded-full ${accent === 'indigo' ? '[&::-webkit-slider-thumb]:bg-indigo-600' : '[&::-webkit-slider-thumb]:bg-primary'} [&::-webkit-slider-thumb]:shadow-sm [&::-webkit-slider-thumb]:transition-all [&::-webkit-slider-thumb]:hover:scale-110 [&::-webkit-slider-thumb]:active:scale-95`}
            aria-label={`Messaging length for ${contextLabel}`}
          />
          </div>
      ...
      <div className={`mb-3 flex gap-2 ${isGenerating || isChatLoading ? 'blur-sm opacity-60' : ''}`}>
        {/* Refine Toggle */}
        {enableRefine && !isGenerating && !isChatLoading && (
          <button
            type="button"
        ...
        {/* Chat Toggle */}
        {enableChat && !isGenerating && !isChatLoading && (
          <button
            type="button"
            disabled={!content || isChatLoading}
            onClick={() => {
      ...
      {enableRefine && isRefineMode && content && (
      ...
      >
        ...
        {!error && (isGenerating || isChatLoading) && !streamedOutput && !chatStreamedOutput ? (
          <div className="mb-2 flex items-center gap-2 text-xs text-body-color">
            <ColorRing
              ...
              height={22}
              width={22}
              ariaLabel={`stream-loading-${settingsScope}`}
        ...
        {!error && !content && !streamedOutput ? (
          <p className="text-xs text-body-color">
            <>No {contextLabel} draft yet. Click <span className="font-semibold">{generateActionLabel}</span> to create one.</>
          </p>
        ) : null}
        <div className="group relative">
          {!isRefineMode && (
            <button
              ...
              data-tooltip-id={`copy-tooltip-${settingsScope}-${id}`}
