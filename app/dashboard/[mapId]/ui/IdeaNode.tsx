...
export default function IdeaNode({ id, data, selected }: NodeProps<IdeaNodeType>) {
  ...
  return (
    ...
    >
      ...
      <Handle type="source" position={Position.Bottom} id="datanode" className="!h-2.5 !w-2.5 !bg-cyan-500" />
      ...
      <div className="relative">
        ...
        <NodeAddPanel
          ...
          actions={[
            {
              label: '+ Data Node',
              description: 'Add data, facts, or metrics',
              onClick: () => setShowDataDropdown(true),
              className:
                'nodrag rounded-xl border border-cyan-200 bg-gradient-to-br from-cyan-50 to-white px-3 py-2 text-left text-xs font-semibold text-cyan-700 transition hover:from-cyan-100 hover:to-cyan-50',
            },
            {
              label: '+ Audience Pain',
        ...
        {showDataDropdown && (
          <div className="absolute left-[calc(100.5%)] top-1/2 z-30 -translate-y-1/2">
            <div className="w-[220px] rounded-xl border border-cyan-200 bg-white p-2 shadow-lg">
              <div className="mb-2 px-2 text-[10px] font-semibold uppercase tracking-[0.14em] text-cyan-700">
                Select Data Type
              </div>
              <button
                type="button"
                className="nodrag w-full rounded-lg border border-cyan-200 bg-cyan-50 px-3 py-2 text-left text-xs font-semibold text-cyan-700 transition hover:bg-cyan-100"
                onClick={() => {
                  setShowDataDropdown(false);
                  mindmap.createDataNodes(id);
                }}
              >
                <div>🤖 AI Suggestions</div>
                <div className="mt-0.5 text-[11px] font-normal text-cyan-600">Generate questions to answer</div>
              </button>
              <button
                type="button"
            ...
            <button
              type="button"
              className="nodrag mt-2 w-full rounded-lg border border-red-200 bg-red-50 px-3 py-1.5 text-center text-xs font-semibold text-red-600 transition hover:bg-red-100"
